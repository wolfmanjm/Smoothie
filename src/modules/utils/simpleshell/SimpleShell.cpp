...
void SimpleShell::test_command( string parameters, StreamOutput *stream)
{
    ...
    if (what == "jog") {
        ...
        step(a, steps, dir, delayus, stream);
...
// similar to test raw but generated by a machine/host so no checking is done (can crash the system if done wrong)
// when done call with done so it can reset axis,
// NOTE this can't be used with corexy or deltas as it drives the actuator directly, it would need to run the steps through the arm solution
// but then figure out how to move in X or Y or Z
//
// step x 32
// step done
void SimpleShell::step_command( string parameters, StreamOutput *stream)
{
    ...
    char ax= toupper(axis[0]);
    uint8_t a= ax >= 'X' ? ax - 'X' : ax - 'A' + 3;
    int steps= strtol(shift_parameter(parameters).c_str(), NULL, 10);
    bool dir= steps >= 0;
    steps= std::abs(steps);

    // used to step microsteps only so 16/32/64 so these can be stepped at full rate (or close to)
    step(a, steps, dir, 20, stream);
}

// issue a raw set of steps at the specified rate in the specified direction
void SimpleShell::step(uint8_t axis, uint32_t steps, bool dir, uint32_t delayus, StreamOutput *stream)
{
    uint32_t tt= delayus*steps; // total time in microseconds

    for(uint32_t s= 0;s<steps;s++) {
        if(THEKERNEL->is_halted()) break;
        THEROBOT->actuators[axis]->manual_step(dir);
        if (tt <= 1000 ) {
            // anything under 1ms is ok to just wait
            wait_us(delayus);
        }else{
