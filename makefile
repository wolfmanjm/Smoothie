...
MRI_UART=MRI_UART_0 MRI_UART_BAUD=$(BAUDRATE)
MRI_BREAK_ON_INIT=1
...
flash: $(PROJECT).hex debug-store
	lpc21isp $< $(CONSOLE) 230400 12000

upload: $(PROJECT).bin debug-store
	dfu-util -d 1d50:6015 -R -D $<

debug: $(PROJECT)_lastupload.elf
	arm-none-eabi-gdb $< --baud $(BAUDRATE) -ex  "set target-charset ASCII" -ex "set remotelogfile mri.log" -ex "target remote $(CONSOLE)"

console:
	stty raw ignbrk -echo  $(BAUDRATE) < $(CONSOLE)
